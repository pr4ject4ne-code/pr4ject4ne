...
/** Keyed by `${doctorId}:${clinicalConditionId}` — migration 029 scopes
 * consent to the exact field, not just the doctor+patient pair (see
 * doctor-consent-db.ts for why). */
...
export function resolveDoctorAttribution(
  ...
  if (!doctorId || !conditionId) return { attributed: false };

  const entry = lookup[`${doctorId}:${conditionId}`];
...
function annotateClinicalConditions(
  ...
  const nextSections = sections.map((section) => {
    ...
    const rows = section.rows.map((row, i) => {
      ...
      const resolution = resolveDoctorAttribution(condition?.doctor_id, condition?.id, lookup);
