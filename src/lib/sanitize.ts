...
export function sanitizeClinicalConditions(
  ...
  for (const item of input.slice(0, maxItems)) {
    ...
    // id: preserve a client-supplied UUID (the form assigns one via
    // crypto.randomUUID() when a condition is first added, same pattern as
    // HospitalDashboardClient's department ids) so doctor-consent requests
    // keep pointing at the same field across edits. Generate one server-side
    // as a safety net for any pre-existing entry that predates this field.
    const rawId = rec.id;
    const id = typeof rawId === 'string' && UUID_RE.test(rawId) ? rawId : randomUUID();
