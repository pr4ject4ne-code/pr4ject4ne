...
/**
 * Object storage for user-uploaded media (hospital photos/logos, first-aid
 * images and short instructional videos), backed by Supabase Storage's REST
 * API — no SDK dependency, matching the project's lean stack. Vercel's
 * filesystem is ephemeral, so local-FS is not an option in production.
 *
...
/**
 * Upload image or video bytes and return a public URL. `prefix` namespaces
 * the object (e.g. 'hospitals', 'first-aid'). Caller must validate size/type
 * first; this re-checks the type as defence in depth.
 */
